...
// middleware to clear cart validation... without this, the warnings won't go away when we remove the offending items from the cart!
...
app.post("/add-to-cart", (req, res) => {
  ...
  Object.keys(req.body).forEach(key =>{
    ...
    const guests = Number(req.body[key]); // number of items going to add
    if (guests === 0) return // no items to add
    // if item not yet in cart, add that item with quantity 0 to cart
